...
{	
	...
	$d->reset();
	$sql_spkhac="select id,ten_$lang,tenkhongdau,thumb,gia,masp,luotxem,mota_vi  from #_product where hienthi= 1 and id_list ='$id_list' and id<>'$id' order by stt asc limit 0,8";
	$d->query($sql_spkhac);
...
?>
 ...
 <div class="col-sm-9 col-lg-9">
        <div class="info chitietsp">
          <h2 class="title_index">Chi tiết sản phẩm</h2>
          <h3><strong><?=$chitiet_sp["ten_vi"]?></strong></h3>
          <div class="col-lg-12 img-asd">
          	<img src="upload/sanpham/<?php if($chitiet_sp["tc_big"]==1) echo $chitiet_sp["photo"]; else echo $chitiet_sp["photo"] ?>" alt="">
          </div>
          <div class="col-lg-12">
          <?=$chitiet_sp["mota_vi"]?>
          </div>
    ...
    <div class="product pdlienquan">
          <div class="col-md-12 col-sm-12 ">
            <h2 class="title_index">Sản phẩm liên quan</h2>
           ...
           ?>
              <div class=" col-md-3 col-sm-3 col-xsm-6 col-xs-12  padding10">
                <div class="item-service"><a href="chi-tiet-san-pham/<?=$result_laylq[$i]['tenkhongdau']?>-<?=$result_laylq[$i]['id']?>.html" class="view-img"></a><a href="chi-tiet-san-pham/<?=$result_laylq[$i]['tenkhongdau']?>-<?=$result_laylq[$i]['id']?>.html" class="view-img"><img src="upload/sanpham/<?php if($result_laylq[$i]["tc_big"]==1) echo $result_laylq[$i]["photo"]; else echo $result_laylq[$i]["photo"] ?>" alt="Thiết bị xử lý nước - nước thải"></a><a href="chi-tiet-san-pham/<?=$result_laylq[$i]['tenkhongdau']?>-<?=$result_laylq[$i]['id']?>.html">
                <h2><?=$result_laylq[$i]["ten_vi"]?></h2>
                  </a> </div>
              </div>
			<?php 
           }
